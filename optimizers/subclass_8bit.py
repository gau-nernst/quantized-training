...
from torchao.dtypes.utils import _dispatch__torch_dispatch__, _implements
...
class OptimState8bit(Tensor):
    ...
    @staticmethod
    def __new__(cls, codes: Tensor, scale: Tensor, qmap: Tensor, signed: bool, stochastic_rounding: bool):
    ...
    def __init__(self, codes: Tensor, scale: Tensor, qmap: Tensor, signed: bool, stochastic_rounding: bool):
    ...
    def __tensor_flatten__(self):
        return self.tensor_attrs, [self.signed, self.stochastic_rounding]
    ...
    def zeros(
        ...
        *,
        ...
        return cls(codes, scale, qmap, signed, stochastic_rounding)

    def __repr__(self):
        return (
            f"{self.__class__.__name__}(signed={self.signed}, block_size={self.block_size}, "
            f"stochastic_rounding={self.stochastic_rounding}, shape={tuple(self.shape)}, "
...
def _(func, types, args, kwargs):
    ...
    if isinstance(dst, OptimState8bit) and isinstance(src, OptimState8bit):
        ...
        codes = quantize_8bit_with_qmap(scaled_src, dst.qmap, stochastic_rounding=dst.stochastic_rounding)
